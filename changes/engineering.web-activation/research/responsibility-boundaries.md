...
The active professional baseline does not authorize production changes, deployment, dependency adoption, access changes, security exceptions, accessibility exceptions, legal claims, risk acceptance, or release approval. It also does not invent a framework, design system, package manager, browser matrix, internal architecture, quality threshold, command, or operational approval workflow. The registered owner and reviewer are `platform-web` [REPO-WEB-DOMAIN-IDENTITY].

## Activation boundary

The owner/reviewer approval under `changes/engineering.web-activation/` resolves the lifecycle decision. Permissions, internal standards, private architecture, product commands, browser and assistive-technology coverage, quality targets, and the security baseline remain project-overlay or task-contract gates. Public standards support professional content but do not establish those local facts or grant operational authority.
